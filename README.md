...
2. サマリーレポート（週間・月間）の通知設定:
   - 実行する関数を選択: `generateWeeklySummary` (週間) / `generateMonthlySummary` (月間)
   - イベントのソースを選択: `時間主導型`
   - 週間の場合: `週ベースのタイマー`、`毎週日曜`、時刻（例：午後9時〜10時）
   - 月間の場合: `月ベースのタイマー`、`特定の日付` (31日など、末日に近い日付)、時刻（例：午後11時〜深夜）
